...
export class RellaxDirective implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.options = { speed: this.speed, center: this.center, percentage: this.percentage};
    ...
    createBlock(el: HTMLElement) {
        ...
        const posY = dataPercentage
            || this.options.center ? (this.scrollableElement.scrollTop ||
        ...
        let percentage = dataPercentage ? dataPercentage : (posY - blockTop + el.parentElement.offsetHeight) /
         (blockHeight + el.parentElement.offsetHeight);
        if (this.percentage == null && this.options.center) { percentage = 0.5; }
