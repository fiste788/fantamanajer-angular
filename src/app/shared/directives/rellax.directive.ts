...
import { afterNextRender, booleanAttribute, Directive, DOCUMENT, ElementRef, inject, input, numberAttribute, PLATFORM_ID, Renderer2 } from '@angular/core';
...
export class RellaxDirective implements OnDestroy, OnInit {
  ...
  public readonly center = input(false, { transform: booleanAttribute });
  public readonly percentage = input(0, { transform: numberAttribute });
  public readonly relativeToWrapper = input(false, { transform: booleanAttribute });
  public readonly speed = input(-3, { transform: numberAttribute });
  public readonly wrapper = input('window');
  ...
  public applyParallaxAnimation(): void {
    ...
    const percentageY = (this.#currentPosY - this.#block.top + this.#screenHeight) / (this.#block.height + this.#screenHeight); // Updated variable name

    const percentageX = (this.#currentPosX - this.#block.left + this.#screenWidth) / (this.#block.width + this.#screenWidth); // Updated variable name
  ...
  public constrainSpeed(number_: number, min: number, max: number): number {
    // Renamed clamp
    if (number_ <= min) {
      return min;
    }
  ...
  public createParallaxBlock(element: HTMLElement): Block {
    // Renamed createBlock
    const dataPercentage = this.#options.percentage;

    const pos = this.getScrollPosition(dataPercentage); // Renamed getPos

    const blockTop = pos.y + element.getBoundingClientRect().top;
    const blockHeight = element.clientHeight || element.offsetHeight || element.scrollHeight;

    const blockLeft = pos.x + element.getBoundingClientRect().left;
    const blockWidth = element.clientWidth || element.offsetWidth || element.scrollWidth;

    let percentageY = 0.5;
    let percentageX = 0.5;
    if (!this.#options.center) {
      // apparently parallax equation everyone uses
      percentageY = dataPercentage || (pos.y - blockTop + this.#screenHeight) / (blockHeight + this.#screenHeight); // Updated screen height

      percentageX = dataPercentage || (pos.x - blockLeft + this.#screenWidth) / (blockWidth + this.#screenWidth); // Updated screen width
    }
  ...
  public getScrollPosition(dataPercentage: number): {
    ...
    let wrapperPosY = this.#options.wrapper?.scrollTop ?? (this.#window.scrollY || this.#document.documentElement.scrollTop || this.#document.body.scrollTop);

    const wrapperPosX = this.#options.wrapper?.scrollLeft ?? (this.#window.scrollX || this.#document.documentElement.scrollLeft || this.#document.body.scrollLeft);

    // If the option relativeToWrapper is true, use the wrappers offset to top, subtracted from the current page scroll.\n
    if (this.#options.relativeToWrapper) {
      const scrollPosY = this.#window.scrollY || this.#document.documentElement.scrollTop || this.#document.body.scrollTop;
  ...
  public performUpdate(): void {
    ...
    if (this.updateScrollPosition() && !this.#isPaused && this.#requestAnimationFrame) {
      ...
      (this.#options.wrapper ?? globalThis).addEventListener('scroll', this.#deferredUpdateHandler, {
        // Updated handler name
        passive: true,
      });

      (this.#options.wrapper ?? document).addEventListener('touchmove', this.#deferredUpdateHandler, {
        // Updated handler name
        passive: true,
      });
    }
  ...
  public scheduleDeferredUpdate(): void {
    ...
    if (this.#deferredUpdateHandler) {
      ...
      (this.#options.wrapper ?? this.#window).removeEventListener('scroll', this.#deferredUpdateHandler); // Updated handler name

      (this.#options.wrapper ?? this.#document).removeEventListener('touchmove', this.#deferredUpdateHandler); // Updated handler name
    }
  ...
  public updateScrollPosition(): boolean {
    ...
    if (this.#options.relativeToWrapper) {
      this.#currentPosY = (df.scrollTop || this.#window.scrollY) - (this.#options.wrapper?.offsetTop ?? 0); // Updated variable name
    }

    if (this.#options.vertical) {
      // Updated variable name
      return oldY !== this.#currentPosY;
    }

    return oldX !== this.#currentPosX && this.#options.horizontal;
  }
}
